...
pub struct WinState {
    ...
    pub backend_context: BackendContext<'static>,

    pub queue: Arc<Queue>,
    pub images: Vec<Arc<Image>>,

    pub swapchain: Arc<Swapchain>,
    pub swapchain_acquire_future: Option<SwapchainAcquireFuture>,
    pub swapchain_needs_recreated: bool,
...
impl WinState {
    pub fn new(
        ...
        let (instance, surface) = create_vulkan_surface(&window);

        let (device_extensions, physical_device, queue_family_index) =
            select_physical_device(&instance, &surface);

        let (device, queue) =
            create_device(device_extensions, &physical_device, queue_family_index);

        let (swapchain, images) = create_swapchain(&window, &device, &surface);

        let (direct_context, backend_context) = create_skia_contexts(&queue);

        let previous_frame_end = sync::now(device.clone()).boxed().into();
    ...
    pub fn recreate_swapchain(&mut self, image_extent: [u32; 2]) {
        (self.swapchain, self.images) = self
            .swapchain
            .recreate(SwapchainCreateInfo { image_extent, ..self.swapchain.create_info() })
    ...
    pub fn resize(&mut self, size: PhysicalSize<u32>) -> bool {
        ...
        self.recreate_swapchain(size.into());
    ...
    pub fn surfaces_mut(&mut self) -> Option<(&mut Surface, &mut Surface)> {
        if self.swapchain_needs_recreated {
            self.recreate_swapchain(self.window.inner_size().into());
        }
            ...
            match acquire_next_image(self.swapchain.clone(), None).map_err(Validated::unwrap) {
                Ok((_, suboptimal, acquire_future)) => {
                    ...
                    if suboptimal {
                        ...
                        // println!("acquire_next_image: suboptimal");
                        // println!("recreate_swapchain = true");
                    }
                }
                Err(VulkanError::OutOfDate) => {
                    self.swapchain_needs_recreated = true;
                    // println!("acquire_next_image: out of date");
                    // println!("recreate_swapchain = true");
                    return None;
                }
                Err(e) => {
        ...
        let image_index = self.swapchain_acquire_future.as_ref()?.image_index();
    ...
    pub fn swap_buffers(&mut self, dirty_rect: BoundingBox) {
        ...
        if self.supports_incremental_present() {
            ...
            let rect = RectangleLayer {
                ...
                extent: [w.max(0.0) as u32, h.max(0.0) as u32],
                layer: 0,
            };
            swapchain_info.present_regions.reserve(1);
            swapchain_info.present_regions.push(rect);
        }
    ...
    fn current_present_mode(&mut self) -> PresentMode {
        if self.vsync && !self.is_moving_or_resizing {
            PresentMode::Fifo
        } else {
...
static VULKAN_LIBRARY: LazyLock<Arc<VulkanLibrary>> = LazyLock::new(|| {
    VulkanLibrary::new() //
...
fn create_vulkan_surface(
...
) -> (Arc<Instance>, Arc<VulkanSurface>) {
    let vulkan_library = VULKAN_LIBRARY.clone();
    ...
    let surface = VulkanSurface::from_window(instance.clone(), window.clone())
...
fn select_physical_device(
...
) -> (DeviceExtensions, Arc<PhysicalDevice>, u32) {
    ...
    (device_extensions, physical_device, queue_family_index)
}
...
fn create_skia_contexts(queue: &Queue) -> (DirectContext, BackendContext<'static>) {
    ...
    // SAFETY: `instance`, `physical_device`, `device`, and `queue` must outlive `backend_context`
    ...
    let direct_context = direct_contexts::make_vulkan(&backend_context, None).unwrap();

    (direct_context, backend_context)
}
...
fn make_get_proc(instance: &Instance) -> &'static impl GetProc {
